...
Checks daemon state before asking for confirmation; `--force` skips the prompt.
An already stopped daemon exits successfully without sending a stop request.
HTTP failures, invalid responses, and a stop response reporting that the daemon
is still running exit with code 1 and an error, without a success message.
Use `sf daemon status` afterward to read back the current state.
...
Both commands use the API's boolean `isRunning` first, then legacy boolean
`running`, then legacy `status` (`running`, `stopped`, or `not_running`). An
explicit `false` takes precedence over conflicting legacy fields. Invalid or
missing state is an error, rather than evidence that the daemon has stopped.
`status --quiet` prints `running` or `stopped`; `--json` preserves the server
payload inside the CLI success envelope.
...
Hand off an active task owned by the calling session. The task returns to the pool with context notes, and the branch/worktree are preserved for the next worker.

The command requires `STONEFORGE_SESSION_ID` (set by the spawner) or an explicit
`--sessionId` from the caller's own session. It never generates a placeholder or
looks up a replacement identity from the task. When `SF_ENTITY_ID` is present,
it must also match the current owner. Closed, deferred, review, backlog,
tombstoned, reassigned, and stale-session handoffs fail without changing the task
or its description. A concurrent task update also rejects the handoff.
Use `sf task reopen` as a separate explicit action for closed tasks.

Prefer the internal session ID. A legacy provider ID is accepted only when it
identifies the current session unambiguously; resumed sessions can share a provider
ID and must use their distinct internal IDs. Older tasks without session history
require an exact match with their stored session ID and consistent ownership.
...
| `-s, --sessionId <id>` | Current caller session ID (defaults to `STONEFORGE_SESSION_ID`) |
